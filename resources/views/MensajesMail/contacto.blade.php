<!DOCTYPE html>
<html>
<head>
<title>Document</title>
</head>
<body align="center">


<p><b>Estimada Familia:</b><br><br>
Junto con saludarlos y agradeciendo su confianza les solicitamos dirigirse al siguiente <a href="{{ env('APP_URL') }}">LINK</a> e iniciar sesión con los siguientes datos.<br>
<b>Usuario: </b>{{ $emailReceptor }}<br>
<b>Contraseña: </b>{{ $clave }}<br><br>
Una vez ingresado al sistema se pide completar el cuestionario con la mayor fidelidad posible. Su propósito es favorecer el proceso de evaluación que está por comenzar junto a su hijo/a.

Muchas gracias!
Equipo Altavida
</p>

</body>
</html>
